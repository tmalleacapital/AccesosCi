...
async function exportarGrupoXlsx(
  ...
  const ws = wb.addWorksheet(grupo.nombre.slice(0, 31));
  ...
  hdrData.eachCell((cell, col) => {
    ...
    cell.border = esSep ? {} : makeBorder(BLANCO);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const asesores = grupo.asesores.filter(
    (a) =>
  ...
  for (const a of asesores) {
    ...
    row.eachCell({ includeEmpty: true }, (cell, col) => {
      ...
      if (!esSep) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        cell.alignment = {
          horizontal: col >= 10 ? 'right' : col >= 4 && col <= 7 ? 'center' : 'left',
        } as any;
        if (col >= 10) {
          cell.numFmt = '#,##0.00';
        }
