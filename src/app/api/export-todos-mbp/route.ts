...
function nombreHojaUnico(base: string, usados: Set<string>): string {
  ...
  let i = 2;
  let candidato = `${limpio.slice(0, 28)} ${i}`;
  while (usados.has(candidato)) {
    i++;
    candidato = `${limpio.slice(0, 28)} ${i}`;
  }
  ...
  try {
    ...
    return new NextResponse(buffer, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="Todos los MBP.xlsx"`,
      },
    });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error('[export-todos-mbp]', msg);
