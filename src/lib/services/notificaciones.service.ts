...
export const RESPONSABLE_JIRA = 'cpeede@capitalinteligente.cl';
...
export function construirCorreoParaJira(
  solicitud: Solicitud,
  plataformas: Plataforma[],
): CorreoSimulado {
  const etiqueta = ETIQUETA_TIPO[solicitud.tipo] ?? solicitud.tipo;
  ...
  return {
    ...
    body: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#111">
        <h2 style="margin-bottom:4px">Paso 3: Crear cuenta en Jira</h2>
        <p style="color:#888;margin-top:0">Ticket <strong>${solicitud.id}</strong> · ${etiqueta}</p>
        <p>El correo corporativo ya fue creado. Por favor crea la cuenta en Jira y luego marca el ticket como completado en el sistema.</p>
